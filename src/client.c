...
urlinfo_t *parse_url(char *url)
{
  ...
  urlinfo_t *urlinfo = malloc(sizeof(urlinfo_t));
    ...
    printf("Path, or everything that comes after the /, is: %s\n", path);
    ...
    *first_backslash = '\0'; // localhost:3490 \0 d20
    ...
    port = first_colon + 1;
    printf("Port, or everything that comes after the colon, is: %s\n", port);
...
{  
  ...
  // While there is still data to receive from the server...
